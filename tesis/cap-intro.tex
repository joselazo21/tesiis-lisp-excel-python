...
\begin{center}
\itshape
?`Puede un lenguaje de dominio espec\'{\i}fico separar la descripci\'on
sem\'antica de una estructura tabular del mecanismo concreto de su
generaci\'on, de modo que el mismo programa declarativo sea v\'alido
independientemente del backend de salida?
\end{center}
\medskip
...
sem\'antica de una estructura tabular de planificaci\'on ---sus tablas, sus
relaciones, sus c\'omputos y sus condiciones de visualizaci\'on condicional---
de los detalles concretos de cualquier backend de generaci\'on.  El backend
es un componente intercambiable que traduce el \'arbol de sintaxis abstracta
a la representaci\'on de salida deseada; el programa del DSL permanece
inalterado al cambiar de backend.  Esta separaci\'on reduce el c\'odigo
necesario, facilita la reutilizaci\'on ante cambios en los datos y hace que
el programa resultante sea legible por el experto del dominio sin conocimiento
de ning\'un sistema de salida espec\'{\i}fico.
...
para la descripci\'on declarativa de estructuras tabulares de planificaci\'on
---tablas relacionadas, columnas calculadas, cuantificaci\'on existencial y
reglas de visualizaci\'on condicional--- con una arquitectura que separa el
DSL del backend de generaci\'on, y demostrar su viabilidad mediante un backend
que produce archivos \xlsx{}.
...
\begin{enumerate}
  ...
  \item Implementar un backend de ejemplo, basado en una funci\'on gen\'erica
    CLOS, que traduzca cada tipo de nodo AST a instrucciones Python para la
    biblioteca openpyxl, generando archivos \xlsx{} con formato condicional
    persistente.
...
familias de nodos, las formas del DSL visibles al programador y su sem\'antica,
y el flujo completo desde la declaraci\'on hasta la entrega del AST al backend.

El cap\'{\i}tulo~\ref{cap:generacion} describe el backend implementado como
demostraci\'on: c\'omo traduce el AST a coordenadas del sistema de salida,
c\'omo compila cada tipo de expresi\'on a cadenas de f\'ormula, y c\'omo
genera las f\'ormulas SUMPRODUCT que implementan el formato condicional
persistente en los archivos \xlsx{}.
