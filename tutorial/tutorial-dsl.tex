...
defensas participa.  En esta tabla el nombre de cada profesor incluye su
grado académico (ej.\ ``MSc.\ Fernando Raúl Rodríguez Flores''), de modo
que las celdas coincidan exactamente con los valores de las columnas de
tribunal en la tabla de defensas para que el \texttt{COUNTIF} pueda
localizarlos.
...
Los roles \texttt{tutor}, \texttt{presidente}, \texttt{secretario},
\texttt{vocal} y \texttt{oponente} son columnas consecutivas.
...
\texttt{(trange defensas-table tutor oponente)}.  Así se cuenta
...
\subsection{Columna calculada con \texttt{:computed}}
    ...
    participa cada profesor.  Como el nombre de cada profesor incluye su grado
    académico (ej. ``MSc.\ Fernando Raúl Rodríguez Flores''), debe coincidir
    exactamente con los valores que aparecen en las columnas de tribunal de la
    tabla de defensas, por lo que el \texttt{COUNTIF} buscará el nombre completo.
    En lenguaje natural: ``para cada profesor, cuenta cuántas filas de la tabla
    de defensas contienen su nombre en cualquiera de los cinco roles''.
    El DSL expresa esto con una columna \emph{calculada}.
...
\texttt{(trange defensas-table tutor oponente)} se refiere a todas las
filas de \texttt{defensas-table} desde la columna \texttt{tutor} hasta la
columna \texttt{oponente}.  Como las cinco columnas de tribunal son
...
Los datos reales corresponden al horario de defensas de tesis de junio
de 2026 de la Facultad de Matemática y Computación de la Universidad de
La Habana.  Se organizan en dos listas: una para cada tabla.

Cada tabla espera dos filas de prefijo (llamadas así porque se colocan
delante de los datos) seguidas de la fila de encabezados y luego las
filas de datos.  Las filas de prefijo se escriben tal cual en el archivo
\texttt{data-tutorial.lisp} y no generan fórmulas; en este tutorial se
dejan vacías.

El nombre de cada profesor incluye el grado académico (ej.\ ``MSc.\ Fernando
Raúl Rodríguez Flores'') para que coincida exactamente con los valores
que aparecen en las columnas de tribunal de la tabla de defensas, lo que
permite al \texttt{COUNTIF} localizarlos correctamente.

\begin{lstlisting}[caption={Datos de defensas-table (13 defensas).}]
...
Las celdas que corresponden a columnas calculadas se marcan como
\texttt{""} en el archivo de datos: el generador las sobreescribe con
la fórmula correspondiente.

\begin{lstlisting}[caption={Datos de profesores-table (32 profesores).}]
...
El nombre del archivo de salida lo define internamente el generador (por
defecto \texttt{Archivo-Excel.xlsx}); no hace falta indicarlo en el DSL.
...
;; TABLA 1: defensas
;; Nueve columnas: dia, hora, estudiante, cinco roles consecutivos
;; (tutor...oponente), local.  Sin coloreado propio.
(def-table defensas-table
  ((dia "") (hora "") (estudiante "") (tutor "")
   (presidente "") (secretario "") (vocal "") (oponente "") (local ""))
  :cell-height 1
...
;; TABLA 2: profesores
;; Columna "defensas" calculada con COUNTIF sobre el rango tutor..oponente.
...
\midrule
Columnas A--I, filas 4--16 & \texttt{defensas-table}: 13 defensas \\
Columna J                  & Oculto (separador) \\
Columnas K--L, filas 4--35 & \texttt{profesores-table}: 32 profesores \\
Columna L, filas 4--35     & Fórmulas \texttt{COUNTIF} generadas \\
Columna M                  & Oculto (separador) \\
Fila 17 (cols.\ A, C)      & Totales de defensas (\texttt{COUNTA}) \\
Fila 36 (cols.\ K, L)      & Total profesores (\texttt{SUM}) \\
Rango K4:K35               & \texttt{FormulaRule}: nombres en rojo
                            si hay conflicto \\
\bottomrule
\end{tabular}
\end{center}

A continuación se muestran las dos tablas como ejemplo de la estructura
que se genera; por limitaciones de espacio se redujo el número de filas
y, en el caso de la tabla de defensas, también el de columnas:
...
\toprule
\textbf{Día} & \textbf{Hora} & \textbf{Estudiante} & \textbf{Tutor} &
\textbf{Presidente} & \textbf{Local} \\
\midrule
05/06/2026 & 10:00
  & Angel Daniel Alonso Guevara
  & MSc. Carmen T. Fernández Montoto
...
08/06/2026 & 10:30
  ...
  & Salón decanato \\
10/06/2026 & 09:30
  & Claudia Hernández Pérez y Joel A. Tamayo
  & MSc. Celia T. González González
  & Dra. Ayme Marrero Severo
  & Postgrado \\
10/06/2026 & 09:30
...
10/06/2026 & 11:30
  ...
  & Salón decanato \\
\bottomrule
\multicolumn{6}{l}{\scriptsize\textit{defensas-table} (6 de 13 filas) —
                     por espacio se omitieron las columnas
                     Secretario, Vocal y Oponente} \\
\end{tabular}
...
\begin{tabular}{lr}
\toprule
\textbf{Nombre} & \textbf{Defensas} \\
\midrule
DrC. Alejandro Piad Morfis               & 5 \\
...
\bottomrule
\multicolumn{2}{l}{\scriptsize\textit{profesores-table} (7 de 32 filas)} \\
\end{tabular}
\end{center}
...
Los cinco roles en columnas consecutivas
  ...
  & Permite \texttt{trange tutor oponente} para contar roles \\
...
(libro nom :hojas)               & Define la aplicación completa \\
...
\texttt{*empty*} & Constante para usar en datos y cabeceras:
  \texttt{(columna *empty*)} o \texttt{("Nombre" *empty*)}.
