...
;; Referencia a una columna de una tabla específica de la región.
;; Evita la ambigüedad cuando dos tablas de la misma región comparten nombre de columna.
;; table-id : símbolo igual al :id de la xl-table destino
;; name     : símbolo DSL de la columna
;; context  : nil | xl-expr-previous-row | xl-expr-next-row
(defclass* xl-expr-table-col-ref () (table-id name context))

;; Rango de columnas de una tabla específica de la región.
;; Paralelo a xl-range pero con resolución limitada a la tabla indicada.
;; table-id : símbolo del id de la tabla
;; from-col, to-col : xl-expr-table-col-ref (to-col puede ser nil → rango de una columna)
(defclass* xl-table-range () (table-id from-col to-col))
...
;; =============================================================================
;; CUANTIFICACIÓN — exists sobre dominios de fila
;;
;; xl-expr-exists comprueba si existe alguna fila en el domain tal que:
;;   - todas las match-keys tienen el mismo valor en la fila actual y en la otra
;;   - algún valor de overlap-cols aparece en ambas filas
;; El generador implementa la comprobación directamente sin construir
;; un árbol de _or/_and/_equals intermedios.
;; =============================================================================

;; Cuantificador existencial estructurado.
;;   bind-var     : símbolo de la variable (documentación, no se usa en evaluación)
;;   domain       : xl-domain-rows
;;   match-keys   : lista de símbolos DSL — columnas que deben coincidir entre filas
;;   overlap-cols : lista de símbolos DSL — columnas donde algún valor debe ser compartido
;;   self-in-cols : solo para dominios cross-table — columnas del domain-table donde
;;                  debe aparecer el valor de la celda actual (nil = exists same-table)
(defclass* xl-expr-exists () (bind-var domain match-keys overlap-cols self-in-cols))

;; Dominio: todas las filas de la tabla, excepto la fila en evaluación.
;;   table-id : nil = tabla actual
(defclass* xl-domain-rows () (table-id))
...
;; =============================================================================
;; POSICIONAMIENTO RELATIVO — para expresiones fijas en la hoja
;; =============================================================================

;; Punto de referencia dentro de una tabla:
;;   table-id    : símbolo del id de la tabla (ej. stats-table)
;;   col-name    : símbolo DSL de la columna ancla
;;   anchor-type : :ultima-fila | :primera-fila
(defclass* xl-anchor () (table-id col-name anchor-type))

;; Navegación desde un ancla: posición final = ancla + secuencia de pasos.
;;   anchor : xl-anchor
;;   steps  : lista de (direccion . n) donde direccion ∈ {abajo arriba izquierda derecha}
(defclass* xl-nav () (anchor steps))

;; Expresión colocada en una posición relativa dentro de la hoja.
;;   pos  : xl-nav — celda destino
;;   expr : nodo de expresión AST — fórmula a emitir
(defclass* xl-sheet-fixed-expr () (pos expr))
...
;; Hoja: nombre + lista de regiones + expresiones en posición relativa
(defclass* xl-sheet () (name regions fixed-expressions))
